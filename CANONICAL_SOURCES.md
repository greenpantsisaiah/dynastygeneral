...
- **The one value seam (`VALUE_MODE`)**: `playerValuesById` is the single scoring number every surface reads (rerank + startable depth + synthesize). It is produced by `scoringValueFor`: in `VALUE_MODE === "market"` (the SHADOW default) it returns `v.value` LITERALLY (float-identical to the pre-seam FantasyCalc passthrough, ignoring the rubric output), so the architecture is wired with provably zero behavior change; in `"rubric"` it returns `evaluate().point_estimate` (which blends back to the market prior when signals are absent, never garbage). Flipping `VALUE_MODE` is the ONLY number-changing step and ships as its own gated PR (canon-keeper + assumption-auditor + snapshot-diff fixtures + founder eyeball; MODEL_LIVE_PLAN Phase D, FORWARD_EV_PLAN Stage 3a). `evaluationById` carries the full per-player `EvaluationOutput`; `priorDrivenById` carries `isRubricPriorDriven(out)` for the "leaning on the market" honesty badge. The shadow guarantee is locked by `evals/priced-pool-shadow.test.ts` (market mode == `v.value`, `Object.is` per key).
