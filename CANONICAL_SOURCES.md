...
- **Season-aware twin (BACKTEST ONLY)**: `getTeamSignalsHistoryForSeason(sb, season)` in `src/lib/signals/team-signals-history.ts` reads `team_signals_history` (migration 0018, keyed `(team, season)`) for ONE past season, shaped like the live `TeamSignalsRow` so the backtest cohort can hand it to the rubric unchanged. The live `getTeamSignalsMap()` (current snapshot) is left untouched; this exists so the WR/QB/TE rubric backtests can join the per-season scheme/coaching the rubrics read (Phase B sig-history). Takes the Supabase client from the caller (no client-construction side effects); never imported by an app surface. `ol_continuity_score` is NOT in the history table (the cohort hydrates it from the per-season nflverse snap-counts pass), so this reader leaves it null. Data populated by `scripts/extract-team-coaching-scheme-history.ts` (LLM, per season, founder-validated) + `scripts/derive-team-metrics.ts --season` (nflverse pbp) merged by `scripts/ingest-team-signals-history.ts` (dry-run default, founder-authorized `--write`). Locked by `evals/team-signals-history.test.ts`.

### Historical OL-grade reader (vintage PFF run/pass grades for backtest)

- **Canonical**: `loadHistoricalOlGrades(sb, predictionYear, knowledgeThrough?)` in `src/lib/signals/historical-ol-grades.ts` (with `normalizePffGrade` + `coerceGrade`). Phase B6 of `MODEL_LIVE_PLAN.md`.
- **Returns**: `Promise<Map<team, { ol_grade_run, ol_grade_pass }>>`, keyed by upper-cased team, values on the rubric-facing 0..1 scale. Empty map when no rows exist (the honest no-data path, so callers run unchanged on free signals).
- **Inputs**: a service-role Supabase client, the decision year, and an optional vintage cutoff (`coded_with_knowledge_through <= cutoff`) for temporal blinding. Reads `historical_signal_codes` (migration 0010), the season-dimensioned backtest store, NOT the current-snapshot `team_signals`.
- **Two stores, one scale**: PFF grades are a LICENSED paid feed (not in nflverse), so the per-season cohort builders cannot derive them like `ol_continuity_score`. The vintage backtest store is `historical_signal_codes` keyed `(team, prediction_year, signal_name)`; the live-read store is `team_signals` (single row per team, no season). A season-S grade writes `prediction_year = S+1`. Both stores hold the NORMALIZED 0..1 value; `normalizePffGrade` divides PFF's native 0..100 at ingest, never at read. `coerceGrade` rejects an un-normalized 0..100 leak (>1).
- **Consumers**: the QB backtest `--with-ol` pass (`scripts/backtest-qb-rubric.ts`), threaded into `buildPositionCohort` via its optional `olGrades` argument. Data populated by two source-distinct ingests, each stamping its own `coded_by` so the audit trail and the idempotent delete-then-insert never cross: `scripts/ingest-pff-ol-grades.ts` (licensed PFF charted grade, `coded_by=ingest:pff-ol-grades:v1`, reads a founder-exported file, normalizes 0..100 -> 0..1) and `scripts/ingest-espn-ol-grades.ts` (FREE ESPN PBWR/RBWR win-rate share, `coded_by=ingest:espn-ol-winrate:v1`, reads the `scrape-espn-block-winrate.ts` output already normalized 0..1). Both founder-authorized write, dry-run default. The QB rubric's `ol_grade_pass` branch is the only rubric consumer today (RB specs it as a variance-band modifier per MODEL_CARD 8.4 but does not yet wire it; WR reads no OL signal). The ESPN proxy is construct-distinct from a PFF grade (win rate, not a charted grade); read its backtest as a rank proxy. Backtest verdict 2026-06-10: ESPN-DB marginal OL lift +0.002 (noise on n=70), confirms no QB OL lift through the live DB read path; see `negative-results.md`.
- **Anti-pattern**: reading `historical_signal_codes` OL rows inline without the vintage filter (a temporal leak), or passing PFF's native 0..100 into a rubric branch that expects 0..1 centered at 0.5. One reader, one normalization point.
- **Bug class avoided**: a backtest that "shows lift" from OL grades coded with hindsight (knowledge after preseason Y), or from a current-only snapshot stamped onto past seasons. Locked by `evals/historical-ol-grades.test.ts`.
...
- **Returns**: `{ available, valueMap, playerValuesById, ktcOverallRanksById, evaluationById, priorDrivenById, valueMode }`. Fetches the realistic available pool (`getAvailableForRequest`), prices EVERY rostered player across the league PLUS the full available pool (`resolvePlayerValues`), resolves the rubric inputs for the same ids via `resolveEnrichedPlayers` (handing down the just-fetched `valueMap`, so NO second FantasyCalc fetch), runs `evaluateForPlayer` per player, selects the SCORING number through the `scoringValueFor(v, out, VALUE_MODE)` seam, reranks `available` by the consensus cascade (`rerankByConsensus`), and annotates the snapshot in place with value-calibrated startable / stable depth (`annotateStartableDepth`).
- **The one value seam (`VALUE_MODE`)**: `playerValuesById` is the single scoring number every surface reads (rerank + startable depth + synthesize). It is produced by `scoringValueFor`. `VALUE_MODE === "rubric"` is the LIVE default (FLIPPED 2026-06-19, the Phase 4 gate): it returns `evaluate().point_estimate`, the four position rubrics blended with the FantasyCalc market prior at `DEFAULT_PRIOR_WEIGHT` (0.55, market-dominant per MODEL_CARD 8.1; blends back to the market prior when signals are absent, never garbage). `VALUE_MODE === "market"` is the SHADOW / revert path: it returns `v.value` LITERALLY (float-identical to the pre-seam FantasyCalc passthrough, ignoring the rubric output), so flipping back to "market" in an incident restores the exact pre-flip numbers. The flip was the ONLY number-changing step and shipped as its own gated PR (dynasty-canon-keeper CRITIQUE + dynasty-assumption-auditor, both GO-with-adjustments; the value-flip snapshot diff `scripts/value-flip-diff.ts` founder-eyeballed). Governance scope (corrected 2026-07-06): those are SAFETY gates (the numbers barely move, nothing breaks). They are NOT the MODEL_LIVE_PLAN Phase D "D3 beats-market" gate, which the flip did NOT clear for any position (no Phase B signal beat the market; `negative-results.md`). The flip is a reversible ARCHITECTURE ship, not evidence the board beats the market; the 0.55 market-dominant weight exists precisely because nothing beat the market. Do not cite the flip as clearing Phase D or FORWARD_EV_PLAN Stage 3a; see the value-pipe deviation entry in MODEL_LIVE_PLAN.md and the Stage 3a note in FORWARD_EV_PLAN.md. The audits' adjustments shipped with the flip: prior weight raised 0.3/0.4 -> 0.55 (matches the card, compresses every mover), TE route-floor slope-discontinuity at 0.60 smoothed to one continuous slope, rookie/year-1 TEs (`years_exp <= 1`) lean harder on the prior (0.7), and the RB cliff-breaker check added to the diff harness (Henry ships at -7: coded `lead_back` not `strict_bellcow`, founder-accepted with the wide RB band). `evaluationById` carries the full per-player `EvaluationOutput`; `priorDrivenById` carries `isRubricPriorDriven(out)` for the "leaning on the market" honesty badge (rendered on the candidate cards + standing call since Phase 3). The seam (both modes) is locked by `evals/priced-pool-shadow.test.ts`: live default is rubric, and market mode still returns `v.value` `Object.is`-identically (the revert guarantee).
- **The seam lives in `src/lib/players/value-mode.ts` (the light module); `priced-pool` re-exports it.** `value-mode` owns `VALUE_MODE`, `scoringValueFor`, and `scoringValueByIds` (the seam for value sites that are NOT the pool: trade pricing, class strength, draft-paths fallback, after-action). It depends only on `values` + `enriched-player` + `evaluation/wiring`, NONE of which cycle back, so the low-level LLM trade-pricing contract (`engine/llm-contract.ts`) can read the seam without the `priced-pool` -> `engine/roster-fit` -> `engine/llm-contract` import cycle. `priced-pool` re-exports the three names so its existing importers + the shadow test are unchanged (the test asserts the re-export is function-identical). Phase 2 (2026-06-19) routed 5 value sites through `scoringValueByIds` (context.ts + llm-contract trade pricing keep FantasyCalc `overall_rank`/`position_rank`, scalar value through the seam; class-strength, draft-paths fallback, aar lane-identity). `admin/evaluate` is the ONE principled exception: it feeds its raw value INTO `evaluate()` as the prior (migrating it is circular), so it stays on `resolvePlayerValues` and will be allowlisted in the Phase G lockdown lint.
...
- **Phase G lockdown lint (2026-06-19)**: now that `VALUE_MODE === "rubric"` is live, the leaf player value has ONE door. The lint "no direct resolvePlayerValues call (use buildPricedPool / the value seam / EnrichedPlayer)" in `evals/anti-patterns.test.ts` bans the `resolvePlayerValues(` CALL outside the canonical boundary (`players/values.ts` definition, `players/value-mode.ts` seam, `players/enriched-player.ts` resolver, `decision-synthesis/priced-pool.ts` pool) plus the ONE documented circular exception (`app/admin/evaluate/page.tsx`, which feeds its raw value INTO `evaluate()` as the prior). It matches the call paren only, so imports, `typeof ...["resolvePlayerValues"]` type references, and prose are unaffected. A new surface that wants a player value builds the priced pool or calls the seam / EnrichedPlayer; it never reaches the raw FantasyCalc fetch again.
...
- **In-season members** (the draft-only beats are blind after the draft; these keep the check-in alive in-season, all grounded): `classifySeasonStandingBeat(standing, stage)` (a record-based milestone over `calcStanding`, the in-season twin of the draft EV-bank milestone; fires only in-season with games played); `classifyCallbackBeats({ openBets, valueOf, ... })` (the running-story head-to-head over OPEN ledger bets, current value via the FantasyCalc `valueOf` map, NON-destructive, never marks a bet resolved); `buildPickResolutions({ openBets, valueOf })` (turns a decisively separated value race, `>= DECISIVE_VALUE_GAP`, into an `ExpectationResolution` for the terminal reconcile); `expectationFromMatchup({ leagueId, season, week, opponentLabel, myPpg, oppPpg })` (the in-season WRITE: a pregame weekly `matchup` bet, metric `points`, `expected_value` = my season ppg, `alternative_value` = the opponent's, the grounded favored signal with no fabricated win-prob model; returns null on a bye / missing ppg); `buildMatchupResolutions({ openBets, currentWeek, resultByBetId })` (the in-season RESOLVE: resolves only PRIOR-week matchup bets from final scores, sets `good` from win/loss and `variance_flag` + `flipped_by` when a favorite lost close or an underdog won close, within `MATCHUP_VARIANCE_MARGIN`). Both feed the same honest-first `reconcileExpectation` trio; a weekly matchup is NEVER `tookContrarian` (you do not choose your opponent, so an underdog loss is neutral, never a critique). The hub (`leagues/[leagueId]/page.tsx`) reads the ledger, reconciles + persists, writes a divergent pick AND this week's matchup as new bets, fetches `getMatchups` for the current + open-bet weeks, and passes `seasonStanding` / `openExpectations` / `valueOf` / `resolvedBeats` to `classifyBeats`.
...
- **Wired from the hub** (`leagues/[leagueId]/page.tsx`, best-effort + RLS-scoped to `authUser`): the WRITE side logs a divergent pick via `expectationFromPickDeviation` + `upsertExpectation` (idempotent on `bet_id`, never re-write an existing id or it resets a resolved row to open); the READ side feeds the callback running-story; the RECONCILE side runs `buildPickResolutions` + `buildMatchupResolutions` -> `reconcileExpectations` -> `persistResolved` only when the value race separates decisively (pick) or a prior week's games are final (matchup). `alternative_player_id` (0016) is required so the callback + reconcile read the call's CURRENT value, not just the draft-time `alternative_value`. The in-season fuel is the weekly matchup loop: `getMatchups` (previously defined but never called) is fetched for the current week (to log the pregame bet) and for any open-matchup-bet week (to resolve it); the pregame write is `expectationFromMatchup` (idempotent on `matchup-{season}-{week}`).
...
### Age effect (position-conditioned aging curve)

- **Canonical**: `ageRetention(position, age)` in `src/lib/players/age-curve.ts`, with three adapter shapes derived from the SAME smooth core: `ageMultiplier(position, age)` (production multiplier, = retention), `ageCurveSigned(position, age)` (signed [-1,+1] youth-dial signal, smooth tanh), and `ageFactor(position, age)` (rank multiplier, lower = better asset; `projectedAgeFactor` ages it forward N seasons). Phase C3 of `MODEL_LIVE_PLAN.md`.
- **Returns**: `ageRetention` is a [0,1] production-retention value, 1.0 across a flat peak shelf `[peakLow, peakHigh]` with C1-smooth Gaussian shoulders (zero slope at the shelf edges, so value AND first derivative are continuous; no stepwise breakpoints). Per-position shapes (peak windows, shoulder sigmas, floors, signed center/spread) live in `AGE_SHAPES`.
- **Inputs**: a position string (QB/RB/WR/TE; non-skill K/DST return the no-op identity) and an age (null returns the no-op: 1.0 for retention/multiplier, 0 for signed, 1.2 for the ageFactor aging prior).
- **Consumers**: `ageMultiplier` -> the RB/WR/TE rubrics (`engine/evaluation/rubrics/`, via a re-export in `engine/evaluation/util.ts`). `ageFactor`/`projectedAgeFactor` -> the available-player pool 3rd-tier dynasty_rank tiebreak (`players/available.ts`), the scout ranking (`scout/score.ts`), and the contender-outlook future projection (`contender-outlook/forecast.ts`; its 1.4/1.8 forecast bands read this scale). `ageCurveSigned` -> the youth_weight Soundboard dial in `decision-synthesis/synthesize.ts` (dial-gated; contributes zero at neutral dials, so the board standing call is unchanged by the curve unless the user moves the dial).
- **Grounding**: peak windows + decline calibrated to RESEARCH_CORPUS.md (Apex per-position peak-age series; Northwestern 2020 RB 28->29 = 25.2% PPG, matched at 24.0%; WR 32+ steepened per MODEL_CARD section 12 named busts; QB base is the population/mid-tier curve respecting the Stathole/Harstad/Apex cliff debunk). dynasty-canon-keeper CRITIQUE 2026-05-29 verdict Defensible/USE; dynasty-assumption-auditor `audit:age-curves` no Indefensible.
- **Distinct (NOT this canonical)**: (1) `qbAgeMultiplier` in `engine/evaluation/rubrics/qb.ts` is a TIER-CONDITIONAL QB curve (the QB rubric never calls `ageMultiplier`); folding QB into one base curve would re-commit the cliff-debunk error the corpus warns against, so it stays separate by design. (2) `winNowAgeSignal` / `futureAgeSignal` (also in `age-curve.ts`, consumed by `windows/compute.ts`) are ROSTER-AGGREGATE signals over a single average age, not the per-player position curve; they live in the same module so all age math is in one tunable home, but they answer a different question. (3) `horizonScore` in `synthesize.ts` is a pool-relative age-vs-median axis, not an aging curve.
- **Anti-pattern**: a new stepwise/binned age curve anywhere (`if (age < 27) return ...` ladders), or re-deriving a position aging multiplier outside this module. Categorical age bins where reality is continuous are at minimum Weak per the assumption-auditor; the canonical is smooth (INVARIANTS.md "Smooth curves, not breakpoints"). Import the adapter you need; do not re-implement.
- **Bug class avoided**: 2026-05-29 architecture audit (Phase C3) found four drifted age-curve implementations: the rank multiplier here, a stepwise production multiplier in `evaluation/util.ts`, a signed [-1,+1] dial curve in `synthesize.ts`, and the roster-aggregate signals in `windows/compute.ts`. A calibration change had to be made in up to four places, and the three per-player curves disagreed (e.g., util's TE bins cliffed at 0.50 by 33 while the rank curve declined gently). Locked by `evals/age-curve.test.ts`.
