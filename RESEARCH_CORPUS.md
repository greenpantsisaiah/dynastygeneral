...
- Last update: 2026-05-24 (PARTIALLY CLOSED open question 12: realized positional-run edge on 41 real startup drafts via `scripts/backtest-positional-run-edge.ts`; Half A directionally confirmed, Half B tier ordering confirmed and strong form corrected). Prior: 2026-05-23 ("draft against a positional run" entry + open question 12 + VALIDATED tier depth via `scripts/validate-positional-run-tier-depth.ts`). 2026-05-21 (validated findings + experiment specs). SEED v1: 2026-04-27.
