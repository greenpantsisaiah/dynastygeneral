...
12. Draft-flow value displacement (positional runs): PARTIALLY CLOSED 2026-05-24, see "VALIDATED (observational): realized positional-run edge in real drafts" below (41 real startup drafts). Half A directionally confirmed; Half B's tier ordering confirmed AND its strong form ("cheaper later") corrected to "waiting on a deep position is ~free, the edge is the off-run discount." League-size calibration ADDED 2026-05-24 (see "VALIDATED (observational): run-edge calibration by league size + format"): segmented by redraft/dynasty/keeper, roster depth, and team count; the draft-cost edge is small and roughly size-independent, the real lever is recoverability slack (`analyzeLeagueRead`). KTC-per-draft-date enrichment RAN but is underpowered (20% snapshot-match coverage, CI crosses zero). Remaining to fully close: broader KTC date coverage + a non-dynasty (seasonal-points) value source for redraft + a random multi-ecosystem sample. Original framing: in real 12-team drafts (KTC / FantasyCalc snapshots), quantify two effects. (a) Half A: does a manager who declines a same-position run bank measurably higher value-over-ADP on the off-run picks taken during the run window? Expected yes; magnitude is the open number. (b) Half B: do that manager's later picks at the run position come at higher value-over-ADP, and is the effect conditioned on the position's tier depth below the run (deep/flat tier = positive, top-heavy/cliff = negative)? Expected: positive only when startable depth remained. This is observable from pick logs plus a value curve (unlike open question 11, which is counterfactual), so it is a backtest, not a simulation. Connects the new "draft against a positional run" common-knowledge entry to real data and to the `survivalPctFor` / `buildLeaguePositionContext` / `buildPositionDepth` canonicals named there.
...
### VALIDATED (observational): realized positional-run edge in real drafts

Closes the observable part of open question 12. `scripts/backtest-positional-run-edge.ts` collects a real corpus of completed Sleeper snake drafts (the founder's own public league ecosystem, since the product DB stores no leagues), derives ADP from the corpus per (season, draft-kind) cohort in ROUNDS (league-size neutral, era-appropriate), detects positional runs (sliding window of 5, 3+ of one position), classifies each manager around a run as CHASE (took the run position) or DECLINE (took another), and measures value-over-ADP (= ADP_round - round_taken; positive = taken later than the player's own market = a discount captured). Read-only public Sleeper API; ADP is corpus-internal, NOT a single anachronistic KTC snapshot.

Corpus: 41 completed startup snake drafts, 8,474 picks, 1,220 run-events (WR 639, RB 422, QB 90, TE 69; WR runs dominate, matching the founder's report). 13 of 43 drafts superflex.

Half A (declining a run banks off-run discount): decliners' off-run picks came at +0.28 rounds of surplus vs chasers' in-run picks at +0.11 rounds; paired per-run-event edge +0.10 rounds [90% CI -0.03, 0.23]. Directional and consistent with the near-mechanical prediction (CI crosses zero; the absolute decliner surplus, ~0.28 rounds = ~3.4 picks at 12-team, is the cleaner signal because value-over-ADP sums to ~0 within a draft so only the relative decliner-vs-chaser gap is meaningful). The mechanism operates in real drafts.

Half B (the run position later, by tier) ordered EXACTLY as the tier-depth value-curve predicted, which is the real test:
- **deep (WR/RB): later minus during = -0.10 rounds [90% CI -0.22, 0.00].** Waiting is approximately FREE.
- **moderate (TE, 1QB QB): -0.47 rounds [90% CI -0.91, -0.03].** Waiting costs real value (~5.6 picks at 12-team).
- **cliffy (SF QB): -0.54 rounds [90% CI -1.16, 0.03]**, thin and noisy (24 run-events, huge SF-QB ADP variance). Directionally the costliest.

Correction to the strong form of Half B: "you get the run position cheaper LATER" is NOT supported even for deep positions; the later pick is statistically indistinguishable from the during-run pick (deep later approx during). The realized edge of declining a run is Half A (the off-run discount) PLUS the fact that waiting on a DEEP position costs essentially nothing, so you bank the off-run discount for free. For SHALLOW positions (TE, QB) waiting actively loses value, so a run there is a genuine warning, not an opportunity. This matches the value-curve validation above (deep = high retained = cheap to wait; shallow/cliffy = low retained = costly) and the Zero-RB framing (the wait works because it is cheap, not because late players are bargains).

Confounders (association, not proven causation): manager skill (decliners may be better drafters), single ecosystem (the founder's leagues share managers and norms, not a random sample), corpus-derived ADP thins in small cohorts. Remaining work to fully close question 12: historical KTC-per-draft-date enrichment (value in market points, not only ADP-rounds) and a multi-ecosystem random sample. Re-run command is in the script header.

### VALIDATED (observational): run-edge calibration by league size + format

The founder's follow-up: a 30-pick dynasty draft has room to rebalance after a run, but a ~15-pick redraft may not (his own counter: maybe it balances because everyone has to cover all positions). `scripts/calibrate-run-by-league-size.ts` segments the real corpus by format type (26 redraft, 11 dynasty, 6 keeper snake drafts), roster depth (rounds), and team count, with ADP computed PER (season, format-type) so redraft and dynasty ADP are separate (the prior backtest pooled them). Value-over-ADP in rounds. Secondary dynasty-only KTC-points cross-check, date-matched to historical snapshots.

Result 1: the DRAFT-COST edge of declining a run is small and roughly format- and size-independent.
- Half A (decliner minus chaser off-run surplus): redraft +0.06 [90% CI -0.05, 0.17], dynasty +0.01 [-0.22, 0.25], keeper +0.41 [0.09, 0.74], all +0.08 [-0.03, 0.18]. A small positive everywhere (significant only in keeper); declining banks a modest off-run discount regardless of size.
- Half B deep (WR/RB later minus during) is near zero in EVERY segment, never positive: redraft -0.12 [-0.23, -0.01], dynasty -0.08 [-0.33, 0.18]; shallow rosters (<=17 rounds) -0.03 [-0.14, 0.08], deep rosters (>=20) -0.21 [-0.40, -0.02]; 10-team -0.19 [-0.35, -0.02], 12-team -0.06 [-0.20, 0.09]. Segment-to-segment differences sit within the noise (shallow-position TE/QB cells are too thin to read, n 9-42, CIs ~1 round). Waiting on a deep position is approximately free across redraft, dynasty, and every team count. This SUPPORTS the founder's counter-intuition: the startable tier depletes at roughly N x starters regardless of roster depth, so the survival of an acceptable player is format-neutral.
- KTC-points cross-check (dynasty deep) was INCONCLUSIVE: +19 KTC points [90% CI -79, +116] at only 20% snapshot-match coverage. Directionally consistent with no late discount, but underpowered. The ktc-per-draft enrichment ran; it needs broader KTC date coverage and a non-dynasty value source for redraft to be decisive.

Result 2: the real league-size lever is RECOVERABILITY (roster slack), which is structural, not an empirical-noise question. Redraft: ~15 picks for ~9 starters = ~6 picks of slack. Dynasty: ~30 picks for ~9 starters now = ~21 picks of slack. This is exactly `analyzeLeagueRead` (`picks_remaining` vs `total_starter_gap`, `unrecoverable_severity`). Smaller slack (redraft, shallow rosters, late in the draft) means a run at a STILL-NEEDED starter position runs out of recovery room sooner, so it becomes "act now" earlier. There is ALSO an outcome-cost asymmetry the rounds-lens cannot see: a late WR in redraft is a replacement-level streamer (near-zero marginal points), in dynasty a rosterable multi-year asset, so the real-roster cost of waiting favors acting sooner in shallow redraft even though the draft-cost surplus does not differ. The founder's dynasty intuition is right for the RECOVERABILITY reason (deep roster = slack), not because waiting is cheaper in value terms; his redraft worry is right ONLY when a run threatens a starter he still needs and slack is tight.

Calibration rule (chase the run vs skip it), routed entirely through existing canonicals, league-size-aware with NO new run signal:
- SKIP (take value elsewhere) when ANY holds: the run position is DEEP (WR/RB; cost of waiting ~free, validated), OR survival of an acceptable player to the next pick is high (`survivalPctFor`; driven by team count and gap), OR recoverability slack is ample (`analyzeLeagueRead`: `picks_remaining` comfortably exceeds `total_starter_gap`; the deep-dynasty case).
- CHASE (take the run position now) only when ALL hold: a real starter gap remains at the position (`build_vs_league` shows below starter need / `edge_at_risk`), AND the position is SHALLOW/CLIFFY (TE, QB especially superflex; cost of waiting real), AND survival is low (big gap / strong demand), AND slack is tight (`unrecoverable_severity` or near it; the shallow-redraft / late-draft case).
- League size enters ONLY through these inputs: team count -> snake gap -> survival; roster depth (R) -> recoverability slack. The empirical data says the raw value-edge does not justify a separate "league size multiplier."

Coach receives the recoverability fields directly: `league_read.structural_constraints` (built by `buildLeagueReadFromSnapshot`, so `picks_remaining` / `total_starter_gap` / `starter_gap_by_position` / `unrecoverable_severity` are tied to the LIVE draft) is in the Coach context payload, the chase-vs-skip rule cites those fields by name, and `league_read` is registered in `REQUIRED_IN_COACH` (evals/anti-patterns.test.ts) so the mirroring lint guards it. Coach grounds the league-size call in exact picks-left-vs-holes numbers, the same recoverability read the trade-guardrail rule uses. (Wired 2026-05-24; previously Coach reasoned from the `my_pick_schedule` + `build_vs_league` proxies only.)
...
- Last update: 2026-05-24 (PM: league-size calibration via `scripts/calibrate-run-by-league-size.ts`, 26 redraft / 11 dynasty / 6 keeper segments + dynasty KTC-per-draft cross-check; draft-cost edge ~size-independent, recoverability slack is the real lever. AM: PARTIALLY CLOSED open question 12: realized positional-run edge on 41 real startup drafts via `scripts/backtest-positional-run-edge.ts`). Prior: 2026-05-23 ("draft against a positional run" entry + open question 12 + VALIDATED tier depth via `scripts/validate-positional-run-tier-depth.ts`). 2026-05-21 (validated findings + experiment specs). SEED v1: 2026-04-27.
