...
Coach today receives `my_pick_schedule` (remaining-pick slack + gaps), `build_vs_league` (starter coverage + edge_at_risk), and `league_position_context` (scarcity), enough to apply the rule. Wiring `analyzeLeagueRead`'s recoverability fields directly into the Coach context (so Coach can cite exact picks_remaining-vs-gap numbers, mirroring the decision endpoints in `engine/context.ts`) is the natural next engineering step.
